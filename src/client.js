...
const previewAcceptHeader = 'application/vnd.airtable.spiderman-preview'
...
const client = mozaik => {
    ...
    const buildApiRequest = (path, params) => {
        const url = config.get('airtable.baseUrl')
        const appId = config.get('airtable.appId')
        const req = request.get(`${url}/${appId}${path}`)

        const paramsDebug = params ? ` ${JSON.stringify(params)}` : ''
        mozaik.logger.info(chalk.yellow(`[airtable] calling ${url}${path}${paramsDebug}`))
        ...
        if (config.get('airtable.token') !== '') {
            req.set('Authorization', `Bearer ${config.get('airtable.token')}`)
        }
    ...
    const apiCalls = {
        ...
        // status() {
        //     const url = 'https://status.airtable.com/api/last-message.json'
        //     let req   = request.get(url)

        //     mozaik.logger.info(chalk.yellow(`[airtable] calling ${url}`))
